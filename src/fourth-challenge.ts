...
class HttpClient {
  static async get({url, method, body}: RequestParameters) {
    return new Promise((resolve, reject) => {
      let request = new XMLHttpRequest();
      request.open(method, url, true);
...
async function procurarFilme(query: string) {
  ...
  console.log(query)
  let result = await HttpClient.get({
    url: `https://api.themoviedb.org/3/search/movie?api_key=${apiKey.value}&query=${query}`,
    method: "GET"
  })
  return result;
}
...
async function adicionarFilme(filmeId: number) {
  let result = await HttpClient.get({
    url: `https://api.themoviedb.org/3/movie/${filmeId}?api_key=${apiKey.value}&language=en-US`,
    method: "GET"
  })
  console.log(result);
}
...
async function criarSessao() {
  let result: any = await HttpClient.get({
    url: `https://api.themoviedb.org/3/authentication/session/new?api_key=${apiKey.value}&request_token=${requestToken}`,
    method: "GET"
  })
  sessionId = result.session_id;
}
...
async function criarLista(nomeDaLista: string, descricao: string) {
  let result = await HttpClient.get({
    url: `https://api.themoviedb.org/3/list?api_key=${apiKey.value}&session_id=${sessionId}`,
    method: "POST",
    body: {
      name: nomeDaLista,
      description: descricao,
...
async function adicionarFilmeNaLista(filmeId: number, listaId: number) {
  let result = await HttpClient.get({
    url: `https://api.themoviedb.org/3/list/${listaId}/add_item?api_key=${apiKey.value}&session_id=${sessionId}`,
    method: "POST",
    body: {
      media_id: filmeId
    }
...
async function pegarLista() {
  let result = await HttpClient.get({
    url: `https://api.themoviedb.org/3/list/${listId}?api_key=${apiKey.value}`,
    method: "GET"
  })
  console.log(result);
}